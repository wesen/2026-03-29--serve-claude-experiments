...
function MacWindow({ title, active, children, width, height, style }) {
  return (
    <div style={{
      width, background: "#fff", border: "2px solid #000",
      boxShadow: "3px 3px 0 #000", display: "flex", flexDirection: "column",
      fontFamily: CHICAGO, ...style,
    }}>
      <div style={{
        height: 22, background: active ? TITLE_BAR_LINES : "#fff",
        borderBottom: "2px solid #000", display: "flex", alignItems: "center",
        padding: "0 4px", gap: 4, flexShrink: 0,
      }}>
        <div style={{ width: 12, height: 12, border: "1px solid #000", background: "#fff", flexShrink: 0 }} />
        <div style={{
          flex: 1, textAlign: "center", fontSize: 11, fontWeight: "bold",
          fontFamily: CHICAGO, background: active ? "rgba(255,255,255,0.8)" : "transparent",
          padding: "0 4px", whiteSpace: "nowrap",
        }}>
          {title}
        </div>
      </div>
      <div style={{ flex: 1, overflow: "hidden", height: height ? height - 22 : undefined }}>
        {children}
      </div>
    </div>
  );
...
function drawLineChart(canvas, data, tooltip) {
  ...
  if (tooltip) {
    ...
    ctx.fillStyle = "#000"; ctx.font = `bold 10px ${CHICAGO}`;
    ctx.textAlign = "left";
...
function drawBarChart(canvas, data, tooltip) {
  ...
  // Grid
  ctx.strokeStyle = "#ccc"; ctx.lineWidth = 1; ctx.setLineDash([2, 2]);
  for (let i = 0; i <= 5; i++) {
    const y = pad.top + (ch / 5) * i;
    ctx.beginPath(); ctx.moveTo(pad.left, y); ctx.lineTo(pad.left + cw, y); ctx.stroke();
  }
  ...
  ctx.lineTo(pad.left, pad.top + ch); ctx.lineTo(pad.left + cw, pad.top + ch); ctx.stroke();

  // Y labels
  ctx.fillStyle = "#000"; ctx.font = `10px ${CHICAGO}`; ctx.textAlign = "right";
  for (let i = 0; i <= 5; i++) {
  ...
  data.labels.forEach((label, gi) => {
    ...
    ctx.fillStyle = "#000"; ctx.textAlign = "center"; ctx.font = `10px ${CHICAGO}`;
  ...
  if (tooltip) {
    ...
    ctx.fillStyle = "#fff"; ctx.strokeStyle = "#000"; ctx.lineWidth = 1;
    ctx.fillRect(tx, ty, tw, th); ctx.strokeRect(tx, ty, tw, th);
    ctx.fillStyle = "#000"; ctx.font = `bold 10px ${CHICAGO}`; ctx.textAlign = "left";
    ctx.fillText(label, tx + 4, ty + 11);
    ctx.font = `10px ${CHICAGO}`;
    items.forEach((item, i) => {
      ctx.fillText(`${item.name}: ${item.value}`, tx + 4, ty + 25 + i * 14);
...
function drawPieChart(canvas, data) {
  ...
  ctx.font = `bold 10px ${CHICAGO}`; ctx.fillStyle = "#000"; ctx.textAlign = "left";
  ...
  values.forEach((val, i) => {
    ...
    ctx.fillStyle = "#000"; ctx.font = `10px ${CHICAGO}`;
...
function ChartCanvas({ chartType, data, width, height }) {
  ...
  const draw = useCallback((tip) => {
    ...
    if (!canvas) return;
    switch (chartType) {
      case "line": drawLineChart(canvas, data, tip); break;
      case "bar": drawBarChart(canvas, data, tip); break;
      case "pie": drawPieChart(canvas, data); break;
      case "scatter": drawScatterChart(canvas, data); break;
    }
  }, [chartType, data]);

  useEffect(() => { draw(tooltip); }, [draw, tooltip]);
    ...
    <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
      {options.map(opt => (
        <label key={opt.value} style={{
          ...
          fontFamily: CHICAGO, fontSize: 11, cursor: "pointer",
        }}>
          <span style={{
            display: "inline-flex", alignItems: "center", justifyContent: "center",
            width: 14, height: 14, border: "2px solid #000", borderRadius: "50%",
            background: "#fff",
          }}>
            {value === opt.value && <span style={{
              width: 8, height: 8, background: "#000", borderRadius: "50%", display: "block",
            }} />}
          </span>
          {opt.label}
        </label>
  ...
  return (
    <div style={{ position: "relative", display: "inline-block" }}>
      <select
        ...
        style={{
          fontFamily: CHICAGO, fontSize: 11, padding: "2px 20px 2px 6px",
          border: "2px solid #000", background: "#fff", borderRadius: 0,
          appearance: "none", cursor: "pointer", boxShadow: "1px 1px 0 #000",
        }}
      >
...
function LegendBar({ series, chartType }) {
  return (
    <div style={{
      ...
      borderTop: "1px solid #000", background: "#f0f0f0",
        ...
        <div key={i} style={{ display: "flex", alignItems: "center", gap: 4 }}>
          {chartType === "line" ? (
            <svg width="22" height="10">
              <line x1="0" y1="5" x2="22" y2="5" stroke="#000" strokeWidth="2"
          ...
          <span style={{ fontFamily: CHICAGO, fontSize: 10 }}>{s.name}</span>
        </div>
      ))}
    </div>
  );
...
export default function ChartWidget() {
  ...
  return (
    <div style={{
      width: "100%", height: "100vh", background: DESKTOP_PATTERN,
      display: "flex", alignItems: "center", justifyContent: "center",
      fontFamily: CHICAGO, padding: 20,
    }}>
      <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
        {/* Menu bar */}
        <div style={{
          height: 22, background: "#fff", borderBottom: "2px solid #000",
          border: "2px solid #000", borderTopLeftRadius: 0,
          display: "flex", alignItems: "center", padding: "0 12px",
          fontSize: 11, fontFamily: CHICAGO, gap: 16, width: CHART_W + 184,
        }}>
          <span style={{ fontWeight: "bold", fontSize: 13 }}>🍎</span>
          <span>File</span><span>Edit</span><span>Chart</span><span>Format</span><span>Window</span>
        </div>

        <div style={{ display: "flex", gap: 0 }}>
          {/* Chart Window */}
          <MacWindow title={`📊 ${datasetKey} — ${chartType.charAt(0).toUpperCase() + chartType.slice(1)} Chart`} active={true} width={CHART_W + 4}>
            <ChartCanvas
              chartType={chartType}
              data={data}
              width={CHART_W}
          ...
          {/* Control Panel */}
          <MacWindow title="⚙️ Options" active={false} width={180} style={{ marginLeft: -2 }}>
            <div style={{ padding: 10, display: "flex", flexDirection: "column", gap: 14 }}>
              {/* Chart type */}
              <div>
                <div style={{
                  fontSize: 10, fontWeight: "bold", fontFamily: CHICAGO,
                  borderBottom: "1px solid #000", paddingBottom: 2, marginBottom: 6,
                }}>Chart Type</div>
                <RadioGroup
                  options={[
              ...
              <div style={{
                ...
                padding: 6, fontSize: 9, lineHeight: 1.4,
                boxShadow: "inset 1px 1px 0 #ccc",
              }}>
                <b>ℹ️ Info</b><br />
                Series: {data.series.length}<br />
                Points: {data.labels.length}<br />
                Max: {Math.max(...data.series.flatMap(s => s.values))}
              </div>

              {/* Decorative */}
              <div style={{
                border: "2px solid #000", height: 48,
                display: "flex", alignItems: "center", justifyContent: "center",
